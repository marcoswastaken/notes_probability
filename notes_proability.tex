...
\begin{document}
		...
		In the specail case that $F$ is the exponential distribution with parameter \(\lambda\), then the failure rate becomes constant with \(h(t)=\gamma\).
		...
		One connection btween gamma and beta distributions is that \(B(a,b)=\dfrac{\Gamma(a)\Gamma(b)}{\Gamma(a+b)}\)
\end{document}